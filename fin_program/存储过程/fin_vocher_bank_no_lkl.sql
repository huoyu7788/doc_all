drop table if exists fin_certificate_no_lkl_detail;
CREATE TABLE `fin_certificate_no_lkl_detail` (
...
DROP PROCEDURE IF EXISTS fin_vocher_bank_no_lkl;  
  delimiter //
  create procedure fin_vocher_bank_no_lkl 
  (
  ...
  set @strsql = 'delete from fin_certificate_no_lkl_detail where pstng_date >= @pstng_date_start and pstng_date <= @pstng_date_end';
  PREPARE slesql FROM @strsql; 
  EXECUTE slesql; 
  DROP PREPARE slesql;

  set @strsql = '
        insert into fin_certificate_no_lkl_detail
        select ''北京'' as name,
...
#call fin_vocher_bank_no_lkl('2017-08-01','2017-08-01,@no_lkl_num);
