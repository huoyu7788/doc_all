drop table if exists fin_certificate_bank_result;
CREATE TABLE `fin_certificate_bank_result` (
...
DROP PROCEDURE IF EXISTS fin_vocher_bank_result;  
  delimiter //
  create procedure fin_vocher_bank_result 
  (
  ...
  if pstng_date_end=''
    ...
    set @strsql = 'delete from fin_certificate_result where dctype like ''BR%''';
  PREPARE slesql FROM @strsql; 
  EXECUTE slesql; 
  DROP PREPARE slesql;

  set @strsql = 'delete from fin_certificate_bank_result where pstng_date >= @pstng_date_start and pstng_date <= @pstng_date_end';
  PREPARE slesql FROM @strsql; 
  EXECUTE slesql; 
  DROP PREPARE slesql;

  set @strsql = '
      insert into fin_certificate_bank_result
      select ''北京'' as name,
             '''' as waybill_no,
             '''' as tax_rate,
             b.doc_type,
        ...
        from
             (
              select pstng_date,
               ...
               from fin_certificate_no_lkl_detail
      ...
      select ''北京'' as name,
        ...
        from
            ...
            select pstng_date,
              ...
              from fin_certificate_lkl_detail
             where name = ''北京''
               and pstng_date >= @pstng_date_start                       
               and pstng_date <= @pstng_date_end
  ...
  set @strsql = '
      ...
      select name,pstng_date,dctype,
        ...
        from fin_certificate_bank_result
       where pstng_date >= @pstng_date_start                       
         and pstng_date <= @pstng_date_end
...
#call fin_vocher_bank_result('2017-08-01','2017-08-01',@bank_num);
