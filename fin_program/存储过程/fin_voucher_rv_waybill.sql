...
DROP PROCEDURE IF EXISTS fin_voucher_rv_waybill;  
  delimiter //
  create procedure fin_voucher_rv_waybill 
  (
  ...
  truncate table fin_certificate_waybill_rv_detail;
  drop table if exists fin_certificate_waybill_rv_detail_1;
  drop table if exists fin_certificate_waybill_rv_detail_2;
  drop table if exists fin_certificate_waybill_rv_detail_3;
  drop table if exists fin_certificate_waybill_rv_detail_4;
  drop table if exists fin_certificate_waybill_rv_detail_5;
  truncate table fin_certificate_waybill_rv_detail_all;
  ...
  set @strsql = '
    ...
    select a.zone_name,a.receipt_id,'''' tax_rate,f.doc_type,f.dctype,f.gl_account as doc_number,
           ...
           a.pstng_date
      from (
            select zone_name,waybill_id, as receipt_id,round(max(final_amount),2) as money,pstng_date,customer,
             ...
             where user_type = 1
             group by zone_name,waybill_id,,pstng_date,customer,pay_type
            ) as a
  ...
  set @strsql = '
    ...
    FROM (
    select zone_name,waybill_id as receipt_id,pstng_date,tax as tax_rate,
     ...
     where type in (0,1)
       and user_type = 1
       and vendor <> ''999999''
     group by zone_name,waybill_id,pstng_date,tax,type 
    union all
    select zone_name,waybill_id as receipt_id,pstng_date,tax as tax_rate,
    ...
    select zone_id,
            zone_name,
            return_id,waybill_id,product_code,real_qty,pstng_date,sale_price,tax,type,vendor
    ...
    select zone_name,waybill_id as receipt_id,pstng_date,tax as tax_rate,
      ...
      from  
    (select zone_id,zone_name,return_id,waybill_id,product_code,real_qty,sale_price,tax,type,pstng_date
       from mds_fin_waybill_return_detail
      where type in (0,1)
        and user_type = 1
        and vendor <> ''999999''
  ...
  set @strsql = '
    ...
    select a.zone_name,a.receipt_id,a.pstng_date,round(sum(money),2) money,flag,vendor 
      from 
    (
    select zone_name,waybill_id as receipt_id,pstng_date,
     ...
     group by zone_name,waybill_id,pstng_date,flag,vendor
    union all
    ...
    group by zone_name,waybill_id,pstng_date,vendor,flag
    ) as a 
 ...
 set @strsql = '
    ...
    select zone_name,waybill_id as receipt_id,pstng_date,
     ...
     where user_type = 1
       and vendor <> ''999999''
     group by zone_name,waybill_id,pstng_date ';
 ...
 set @strsql = '
    ...
    from (
    select zone_name,pstng_date,waybill_id as receipt_id,
     ...
     where user_type = 1
       and vendor <> ''999999''
     group by zone_name,waybill_id,pstng_date,type,tax 
    ) as a
    join 
  ...
  set @strsql = '
    insert into fin_certificate_waybill_rv_detail_all 
    ...
    from (
        ...
        from (
            ...
            from (
                ...
                from fin_certificate_waybill_rv_detail_all as a
  ...
  truncate table fin_certificate_waybill_rv_detail_1;
  truncate table fin_certificate_waybill_rv_detail_2;
  truncate table fin_certificate_waybill_rv_detail_3;
  truncate table fin_certificate_waybill_rv_detail_4;
  truncate table fin_certificate_waybill_rv_detail_5;
