...
select '北京' as zone_name,
       ...
       concat(pstng_date,b.dd_name) as text,
  ...
  from
       (
        select pstng_date,
         ...
         from default.mds_fin_u8_third_pay_no_lkl as a
        where dt = '$day'
...
select '北京' as name,
       ...
       b.dctype as dctype,
       --case when a.flag = 'BR_BJ_YFZK_LKL_CASH' then ''
...
select '杭州' as name,
       ...
       b.doc_type as doc_type,
       b.dctype as dctype,
       --case when a.flag = 'BR_HZ_YFZK_LKL_CASH' then ''
       b.gl_account as doc_number,
       b.dd as dd,
       '' as project,
       '' as project_cate,
...
select '链商中国' as name,
       ...
       '' as project,
       '' as project_cate,
       case when a.system = 'BR_LSZG_TJ_SXF' then a.money_sxf_tj_lkl
            when a.system = 'BR_LSZG_HZ_SXF' then a.money_sxf_hz_lkl
            when a.system = 'BR_LSZG_TJ_NBWL' then a.money_nbwl_cys_tj
            when a.system = 'BR_LSZG_HZ_NBWL' then a.money_nbwl_cys_hz
       end as money,
       concat(lkl_date,' ',b.dd_name) as text,
       pstng_date as pstng_date,
       '' as customer,
