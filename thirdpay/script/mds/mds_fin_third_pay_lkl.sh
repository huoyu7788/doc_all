  ...
  from
      ...
      select 'LKL' as module,
       ...
       where name = '北京'
         ...
         -- and module like 'LKL%'
         and module in ('LKL','LKL_CASH','WX_SM_CASH','ZFB_SM_CASH')
         and dt = '$day'
  ...
  from
      ...
      select 'LKL' as module,
       ...
       group by substr(lkl_trans_time,1,10),
        ...
        join
      default.mds_fin_u8_third_pay_dimension as b
      on (concat('BR_BJ_LSZG_TJ_SXF_',substr(a.module,1,3)) = b.flag)
union all
--手续费杭州
select '北京' as name,
       0 as money_sxf_bj_lkl,
  ...
  from
      ...
      select 'LKL' as module,
       ...
       group by substr(lkl_trans_time,1,10),
        ...
        join
      default.mds_fin_u8_third_pay_dimension as b
      on (concat('BR_BJ_LSZG_HZ_SXF_',substr(a.module,1,3)) = b.flag)
union all
--应收账款-北京
select '北京' as name,
       0 as money_sxf_bj_lkl,
...
select '北京' as name,
       ...
       0 as money_nbwl_cys_hz,
       'BR_BJ_YFZK' as system,
       -- nvl(c.f_code,'01010013') as cy_code_q,
  ...
  from
      ...
      select 'LKL_CASH' as module,
       ...
       where name = '北京'
         -- and module = 'LKL_CASH'
         and module in ('LKL_CASH','WX_SM_CASH','ZFB_SM_CASH')
         and name is not null
         and dt = '$day'
       group by substr(lkl_trans_time,1,10),
                substr(transaction_time,1,10),
      ...
      (select waybill_no, f_code 
        from(select waybill_no, wumartcompany_id, zone_id, trans_uid 
               from ods.ods_tms_order_waybill where dt = '$day'
            ) wb 
        join(select uid, company_id 
               from ods.ods_tms_trans_user where dt = '$day'
            ) user on user.uid = wb.trans_uid
        join(select company_id, f_code, zone_id 
               from ods.ods_tms_trans_company where dt = '$day'
...
select '天津' as name,
       ...
       0 money_nbwl_cys_hz,
       'BR_TJ_YFZK' as system,
       -- nvl(c.f_code,'02010003') as cy_code_q,
...
select '杭州' as name,
       ...
       0 money_nbwl_cys_hz,
       'BR_HZ_YFZK' as system,
       -- nvl(c.f_code,'03010001') as cy_code_q,
